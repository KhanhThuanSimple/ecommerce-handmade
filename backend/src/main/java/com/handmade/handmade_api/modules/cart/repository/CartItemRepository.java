...
public interface CartItemRepository extends JpaRepository<CartItem, Long> {
    ...
    @Query(value = "SELECT ci.product_id AS productId, p.name AS productName, " +
            "p.base_price AS price, pi.image_url AS imageUrl, SUM(ci.quantity) AS quantity, " +
            "CAST(COALESCE((SELECT SUM(pv.inventory) FROM product_variants pv WHERE pv.product_id = ci.product_id), 0) AS INTEGER) AS maxInventory " +
            "FROM cart_items ci " +
            "INNER JOIN carts c ON ci.cart_id = c.id " +
            "INNER JOIN products p ON ci.product_id = p.id " +
            "LEFT JOIN product_images pi ON p.id = pi.product_id AND pi.is_featured = TRUE " +
            "WHERE c.user_id = :userId " +
            "GROUP BY ci.product_id, p.name, p.base_price, pi.image_url",
            nativeQuery = true)
